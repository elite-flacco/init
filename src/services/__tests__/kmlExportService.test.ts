...
describe("KMLExportService", () => {
  ...
  it("should generate valid KML content", async () => {
    ...
    expect(kml).toContain(
      "<description>Your personalized travel plan for Tokyo, Japan</description>",
    );
  ...
  it("should properly escape XML characters", async () => {
    ...
    const kml = await KMLExportService.generateKML(mockPlanWithSpecialChars, {
      useRealCoordinates: false,
    });

    expect(kml).toContain(
      "Place with &lt;special&gt; &amp; &quot;characters&quot;",
    );
  ...
  it("should handle null or undefined plan gracefully", async () => {
    await expect(
      KMLExportService.generateKML(null as unknown as EnhancedTravelPlan),
    ).rejects.toThrow();

    await expect(
      KMLExportService.generateKML(undefined as unknown as EnhancedTravelPlan),
    ).rejects.toThrow();
  });
  ...
  it("should handle missing destination gracefully", async () => {
    ...
    await expect(
      KMLExportService.generateKML(
        planWithoutDestination as EnhancedTravelPlan,
      ),
    ).rejects.toThrow();
  });
  ...
  it("should handle missing destination name gracefully", async () => {
    ...
    await expect(
      KMLExportService.generateKML(planWithIncompleteDestination),
    ).rejects.toThrow();
  });
});
