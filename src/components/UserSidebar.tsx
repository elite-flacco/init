...
const UserSidebar = React.forwardRef<UserSidebarRef, UserSidebarProps>(
  ({ isOpen, onClose, onOpenAuthModal }, ref) => {
    ...
    }, [user, loadSavedPlans, loadSavedDestinations]);
    ...
    const handleViewSavedPlan = async (plan: SavedPlan) => {
      ...
      try {
        ...
        const fullPlanResponse = await makeAuthenticatedRequest(
          `/api/user/plans/${plan.id}`,
        );
      ...
      try {
        ...
        if (deleteDialog.type === "plan") {
          ...
          setSavedDestinations((prev) =>
            prev.filter((d) => d.id !== deleteDialog.item!.id),
          );
        }
    ...
    const savePlan = async () => {
      ...
      try {
        ...
        setSavedPlans((prev) =>
          prev.map((plan) =>
            plan.id === editingPlan.id ? { ...plan, ...updateData } : plan,
          ),
        );
    ...
    const saveDestinationNotes = async () => {
      ...
      try {
        const response = await makeAuthenticatedRequest(
          `/api/user/destinations/${editingDestination.id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
    ...
    if (!user && !loading) {
      return (
        ...
        >
          ...
          {/* Sidebar */}
          <div
            className={`absolute right-0 top-0 h-full w-full max-w-md bg-background border-l border-border shadow-2xl transform transition-transform duration-300 ${isOpen ? "translate-x-0" : "translate-x-full"}`}
          >
            {/* Header */}
              ...
              <div className="text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-foreground-secondary mb-6">
                    Create an account to save your travel plans, bookmark
                    destinations, and access them anytime.
                  </p>
                </div>
                <button
    ...
    return (
      ...
      >
        ...
        {/* Sidebar */}
        <div
          className={`absolute right-0 top-0 h-full w-full max-w-md bg-background border-l border-border shadow-2xl transform transition-transform duration-300 ${isOpen ? "translate-x-0" : "translate-x-full"} flex flex-col`}
        >
          {/* Header */}
                      ...
                      {filteredPlans.map((plan) => (
                        <div
                          ...
                          onClick={() =>
                            ...
                            !editingPlan &&
                            handleViewSavedPlan(plan)
                          }
                          className={`relative bg-background-soft shadow-md rounded-lg p-3  transition-colors group ${
                            openingPlanId === plan.id
                    ...
                    <div className="space-y-3">
                      {filteredDestinations.map((item) => (
                        <div
                          ...
                          onClick={() =>
                            !editingDestination &&
                            handleViewDestinationDetails(item)
                          }
                          className={`relative bg-background-soft shadow-md rounded-lg p-3 transition-colors group ${
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-background-soft rounded-lg p-4 shadow-lg transition-colors group">
                        ...
                        <div className="text-xs font-medium text-foreground-secondary uppercase tracking-wide">
                          Trips
                        </div>
                      </div>
                      <div className="bg-background-soft rounded-lg p-4 shadow-lg transition-colors group">
